...
export const Header = () => {
	const titleTransition =
		"hover:-translate-y-0.5 duration-500 inline-block transition-all ease-out";
		...
		<header className="flex items-center mt-20 md:justify-center overflow-x-hidden overflow-y-clip min-h-screen">
			...
			>
				<div className="w-full max-w-128">
					...
					"
					>
						<span className={`font-bold ${titleTransition}`}>
							Real time
							<span className="text-primary inline-block">*</span>
						</span>{" "}
						<span className={titleTransition}>stock</span>{" "}
						<span className={titleTransition}>data</span>{" "}
						<span className={titleTransition}>at</span>{" "}
						<span className={titleTransition}>your</span>{" "}
						<span className={titleTransition}>fingertips.</span>
					</h1>
				...
				>
					...
					<SearchBar
						className="mt-16 mx-auto md:m-0 max-w-96 min-w-48"
						sqHeight={16}
					/>
					<LogoVisual
						className="scale-100 absolute opacity-50 -rotate-90 -z-10 -top-12 left-8 
