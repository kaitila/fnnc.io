...
export const StockViewMedium = ({
	...
	useEffect(() => {
		...
		getPriceQuote(ticker)
			.then(({ price, changeP }) =>
				setData({ price: price, changeP: changeP })
			)
	...
	return (
		<Link href={`/dashboard/company/${ticker}`}>
			<div
				className={`shadow-sm border-primary border rounded-xl col-span-1 ${className}`}
			>
					...
					<div className="transition-all duration-300">
						{pending ? (
							...
							<>
								...
								<h3
									className={`text-lg font-semibold ${positive ? "text-green-500" : "text-red-500"} text-nowrap`}
								>
									{positive ? (
