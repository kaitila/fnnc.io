...
import { Candle } from "@/components/stocks/card/Candle";
...
export const Chart = async ({ ticker, chartWidth = 20 }: ChartProps) => {
	if (chartWidth < 0) {
		chartWidth = 0;
	}
	...
	return (
		<>
			<div className="flex gap-0.5 h-28">
				{stockData
					? stockData["Time Series (Daily)"].map((elem) => (
							<Candle
								open={
									((Number(elem["1. open"]) - low) / delta) *
									100
								}
								close={
									((Number(elem["4. close"]) - low) / delta) *
									100
								}
								key={elem["6. date"]}
							/>
